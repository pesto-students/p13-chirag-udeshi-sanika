...
function makeComputerMove() {
	...
	const availableButtons = buttonsArray.filter(
		button => {
			return button.innerHTML === "";
		}
	);
	//select a random button from available ones
	let randomIndex = Math.floor(Math.random() * availableButtons.length);
	availableButtons[randomIndex].innerHTML = 'O';
	availableButtons[randomIndex].setAttribute("disabled", true);
	switchTurn();
